...
On this machine the Simulator is the required milestone. Unsigned Simulator builds (`CODE_SIGNING_ALLOWED=NO`) have **no CloudKit entitlement**; asking SwiftData for a private CloudKit database SIGTRAPs. The app therefore uses a local-only store unless a signed `embedded.mobileprovision` contains `iCloud.app.perimedi.ios`.
