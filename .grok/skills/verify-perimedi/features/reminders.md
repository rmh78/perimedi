...
| Control | ID | Proof |
|---|---|---|
| Banner | `reminder.banner` | `testDoseReminderTaken` waits up to 12s. |
| Taken | `reminder.taken` | Banner gone; `cycle.lane.{slug}.status` becomes `taken`. |
| Snooze | `reminder.snooze` | Banner dismisses without taking. |
