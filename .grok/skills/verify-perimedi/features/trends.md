...
UI tests that need several cycles of scores launch with `-fixture=trends` (not `-loadSample`). Empty store still shows the screen with need-cycles copy. Every catalog symptom is on screen, grouped, selected or not.
