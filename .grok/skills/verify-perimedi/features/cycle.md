...
- First-use only asserts `cycle.effect` absent on empty home and `no-previous` after one period. Two-cycle `id:down` / `id:worse` values are domain tests (`EffectLogicTests`); journey tests do not load sample.
